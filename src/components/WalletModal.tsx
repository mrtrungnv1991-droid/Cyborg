...
export const WalletModal: React.FC<WalletModalProps> = ({
  ...
  const handleConfirmDeposit = () => {
    ...
    setTimeout(() => {
      ...
      setSuccessMsg(`✓ Đã nạp thành công +${formatCurrency(depositAmount, user.currency)} vào ví Escrow!`);
      setTimeout(() => {
        setSuccessMsg('');
      }, 3000);
    }, 800);
  ...
  const handleConfirmWithdraw = (e: React.FormEvent) => {
    ...
    if (!withdrawAccountNo.trim()) {
      alert('Vui lòng nhập số tài khoản / ví nhận tiền!');
      return;
    }
    ...
    setSuccessMsg(`✓ Đã gửi yêu cầu rút ${formatCurrency(withdrawAmount, user.currency)} về ${withdrawBank} lên Admin Panel xét duyệt!`);
    setTimeout(() => {
      setSuccessMsg('');
    }, 4000);
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold font-mono text-white flex items-center gap-2">
                <span>VÍ ĐIỆN TỬ BẢO LÃNH ESCROW</span>
              ...
              <p className="text-xs text-slate-400 font-mono mt-0.5">
                Nạp & rút tiền tự động 24/7, bảo lãnh gom đơn không lo rủi ro
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            <div className="p-4 rounded-xl bg-gradient-to-br from-slate-900 to-[#111624] border border-slate-800">
              <div className="text-xs text-slate-400 flex items-center justify-between">
                ...
                <span className="text-[10px] text-emerald-400">Rút được ngay</span>
              </div>
              <div className="text-2xl font-black text-emerald-400 mt-1">
                {formatCurrency(user.walletBalance, user.currency)}
              </div>
              <div className="text-[11px] text-slate-400 mt-1 flex items-center gap-1">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> Sẵn sàng khóa slot & rút về bank
              </div>
            </div>

            <div className="p-4 rounded-xl bg-gradient-to-br from-slate-900 to-[#111624] border border-slate-800">
              <div className="text-xs text-slate-400 flex items-center justify-between">
                <span>Đang Khóa Trong Pool Gom:</span>
              ...
              <div className="text-[11px] text-amber-400 mt-1 flex items-center gap-1">
                <ShieldCheck className="w-3.5 h-3.5" /> Tự động hoàn lại nếu pool không đủ người
              </div>
            </div>
          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                  ...
                  >
                    ...
                    <div className="text-[10px] text-slate-400 mt-0.5">0% Phí</div>
                  </button>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-black/60 border border-slate-800 flex flex-col sm:flex-row items-center gap-4">
                ...
                <div className="space-y-1 text-xs text-slate-300">
                  <div className="text-white font-bold flex items-center gap-1.5">
                    ...
                    <span>Nạp Tiền Nhanh Qua Cổng Thanh Toán Test</span>
                  </div>
                  <p className="text-[11px] text-slate-400 leading-relaxed font-sans">
                    Bấm <strong>Xác Nhận Nạp Ngay</strong> bên dưới để nạp tiền vào ví Escrow.
                  </p>
                  <div className="text-[10px] text-emerald-400">
                    Mã GD: {generateTxHash()}
                  </div>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleConfirmWithdraw} className="space-y-4">
              <div className="p-3.5 rounded-xl bg-cyan-950/30 border border-cyan-500/30 text-[11px] text-cyan-300 font-sans space-y-1">
                <div className="font-bold flex items-center gap-1.5 font-mono text-cyan-200">
                  ...
                  <span>QUY ĐỊNH RÚT TIỀN TỪ VÍ CYBERPOOL</span>
                </div>
                <p>
                  • Hạn mức rút tối thiểu: <strong>50.000 ₫</strong>. Miễn phí rút tiền 0%.
                </p>
                <p>
                  • Lệnh rút tiền được chuyển ngay lập tức lên <strong>Root Admin Panel</strong> để kế toán kiểm tra & giải ngân qua VietQR trong vòng 5 - 15 phút.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <div className="text-[10px] text-slate-500">
                    Tối đa có thể rút: {formatCurrency(user.walletBalance, user.currency)}
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  <label className="text-[11px] text-slate-400 font-bold">
                    {withdrawMethod === 'crypto_ltc'
                      ? 'Địa Chỉ Ví Litecoin LTC Core (*):'
                      : withdrawMethod === 'binance_pay'
                      ? 'Binance Pay ID hoặc Binance UID (*):'
                      : withdrawMethod === 'usdt'
                      ? 'Địa Chỉ Ví USDT (*):'
                      : 'Số Tài Khoản / Số Ví (*):'}
                  </label>
                  <input
                    ...
                    placeholder={
                      withdrawMethod === 'crypto_ltc'
                        ? 'VD: LZeE2hL9qHSmV7gJ2wH7QG9Z2C81uYyX3w...'
                        : withdrawMethod === 'binance_pay'
                        ? 'VD: 582910384 (Pay ID) hoặc 293847291 (UID)...'
                        : withdrawMethod === 'usdt'
                        ? 'VD: TWYvQ5X4h3uC48K8kS1mN7kY6Q3kH2g9aB...'
                        : 'VD: 0912345678...'
                    }
                ...
                <div className="col-span-1 sm:col-span-2 space-y-1">
                  <label className="text-[11px] text-slate-400 font-bold">Tên Chủ Tài Khoản Thụ Hưởng (*):</label>
                  <input
                    type="text"
                    required
              ...
              >
                ...
                <span>Gửi Yêu Cầu Rút Tiền Về Ngân Hàng</span>
              </button>
            </form>
          )}
          ...
          {activeTab === 'history' && (
            <div className="space-y-3">
              <div className="text-xs font-bold text-white uppercase flex items-center justify-between">
                <span>LỊCH SỬ CÁC YÊU CẦU RÚT TIỀN CỦA BẠN</span>
                <span className="text-[10px] text-cyan-400">Đồng bộ trực tiếp với Admin Panel</span>
              </div>
        ...
        {activeTab === 'deposit' && (
          <div className="p-4 sm:p-5 border-t border-slate-800 bg-[#0a0d14] flex items-center justify-between font-mono">
            <div className="text-xs text-slate-400">
              Số tiền nạp: <span className="text-cyan-400 font-bold">{formatCurrency(depositAmount, user.currency)}</span>
            </div>
            ...
            >
              ...
              <span>{isProcessing ? 'Đang Xử Lý...' : 'Xác Nhận Nạp Ngay'}</span>
            </button>
          </div>
        )}
      </div>
