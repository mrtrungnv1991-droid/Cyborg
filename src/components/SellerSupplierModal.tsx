      ...
      <div className="relative w-full max-w-4xl rounded-xl bg-[#0a0d16] border border-cyan-500/40 shadow-[0_0_40px_rgba(6,182,212,0.2)] overflow-hidden my-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold font-mono text-white flex items-center gap-2">
                <span>KÊNH BÁN SỈ & KẾT NỐI API SUPPLIERS (ADCP OS)</span>
              ...
              <p className="text-xs text-slate-400 font-mono mt-0.5">
                Đồng bộ kho hàng tự động, nạp API nhà cung cấp & kiểm duyệt cron/checklive
              </p>
            </div>
          </div>
        ...
        <div className="flex border-b border-slate-800 bg-slate-950/60 px-4 sm:px-6">
          ...
          >
            ...
            <span>API NHÀ CUNG CẤP ({suppliers.length})</span>
          </button>
          ...
          >
            ...
            <span>KIỂM DUYỆT BATCH KEY</span>
          </button>
          ...
          >
            ...
            <span>DOANH THU & ĐỐI SOÁT VÍ</span>
          </button>
        </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800">
                  <div className="text-xs font-mono text-slate-400">Tổng Số Dư Nhà Cung Cấp:</div>
                ...
                <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800">
                  <div className="text-xs font-mono text-slate-400">Tần Suất Checklive:</div>
                ...
                <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800">
                  <div className="text-xs font-mono text-slate-400">Tỷ Lệ Thành Công API:</div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-cyan-400 border border-cyan-500/30 font-mono text-xs flex items-center gap-1.5 transition-all"
                        >
                          <RefreshCw className={`w-3.5 h-3.5 ${syncingId === sup.id ? 'animate-spin text-cyan-300' : ''}`} />
                          <span>{syncingId === sup.id ? 'Đang đồng bộ...' : 'Đồng Bộ Tức Thì'}</span>
                        </button>
                      </div>
                    </div>
              ...
              <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-mono font-bold text-slate-200">
                    DÁN DANH SÁCH KEY HÀNG LOẠT (MỖI KEY 1 DÒNG)
                  </label>
                  <span className="text-xs text-slate-400 font-mono">Tự động check trùng & format</span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <button
                    ...
                    className="py-2 px-4 rounded-lg bg-cyan-500 hover:bg-cyan-400 text-black font-mono font-bold text-xs uppercase"
                  >
                  ...
                  {validationResult && (
                    <div className="flex items-center gap-4 text-xs font-mono">
                      <span className="text-slate-300">Tổng: <strong>{validationResult.total}</strong></span>
                      <span className="text-emerald-400">Hợp lệ: <strong>{validationResult.valid}</strong></span>
                      <span className="text-amber-400">Trùng lặp: <strong>{validationResult.duplicates}</strong></span>
                    </div>
                  )}
                </div>
              </div>
          ...
          {activeTab === 'payouts' && (
            <div className="space-y-4">
              <div className="p-6 rounded-xl bg-gradient-to-r from-cyan-950/40 via-slate-900 to-slate-900 border border-cyan-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <div className="text-xs font-mono text-slate-400">Doanh Thu Đã Giải Phóng Escrow:</div>
                  <div className="text-2xl font-black font-mono text-cyan-400 mt-1">
                    {formatCurrency(38400000, user.currency)}
                  </div>
                  <div className="text-[11px] text-slate-400 font-mono mt-1">
                    Hoa hồng sàn: 2.5% • Tiền về tài khoản ngân hàng trong 15 phút
                  </div>
                </div>

                <button
                  onClick={() => alert('Yêu cầu rút tiền đối soát CTV đã được gửi tới bộ phận Tài Chính.')}
                  className="py-3 px-6 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-black font-mono font-bold text-xs uppercase tracking-wider transition-all"
                >
                  Rút Tiền Đối Soát Ngay
                </button>
              </div>
            </div>
          )}
        ...
        <div className="p-4 sm:p-6 border-t border-slate-800 bg-[#070a12] flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="py-2.5 px-6 rounded-lg bg-slate-800 hover:bg-slate-700 text-white font-mono text-xs font-bold"
          >
            Đóng Quản Trị
          </button>
        </div>
