          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold font-mono text-white tracking-wide">
                {completedOrders ? 'ĐẶT MUA THÀNH CÔNG // BUNG KEY TỨC THÌ' : 'XÁC NHẬN ĐƠN HÀNG MUA LẺ'}
              </h2>
              <p className="text-xs text-slate-400 font-mono mt-0.5">
                {completedOrders 
                  ? 'Mã bản quyền số đã được bàn giao và lưu an toàn vào Kho Key (Vault)' 
                  : 'Kiểm tra kỹ danh sách sản phẩm & số tiền thanh toán để tránh nhấp nhầm'}
              </p>
            </div>
          </div>
            ...
            <div className="space-y-5 animate-in zoom-in-95 duration-200">
              <div className="p-4 rounded-xl bg-emerald-950/60 border border-emerald-500/50 flex items-center gap-3 text-emerald-300">
                ...
                <div>
                  <h3 className="text-sm font-bold font-mono text-white">
                    Giao Dịch Thành Công! Đã trừ tiền ví & bung {completedOrders.length} mã key
                  </h3>
                  <p className="text-xs font-mono text-emerald-400 mt-0.5">
                    Số dư ví hiện tại: <strong className="text-white">{formatCurrency(currentUser.walletBalance, currentUser.currency)}</strong>. Mã key đã được lưu vào <strong className="text-cyan-300">Kho Key (Vault)</strong> và ghi vào <strong className="text-cyan-300">Sổ Cái Ví</strong>.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <h4 className="text-xs font-bold font-mono text-cyan-400 uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  <span>DANH SÁCH KEY BẢN QUYỀN SỐ ĐÃ BÀN GIAO:</span>
                </h4>
                  ...
                  {completedOrders.map((order, idx) => (
                    ...
                    >
                      ...
                      <div className="text-[10px] font-mono text-slate-400 flex items-center justify-between">
                        <span>Mã giao dịch: <strong className="text-slate-300">{order.txId}</strong></span>
            ...
            <div className="space-y-4">
              ...
              <div className="p-3.5 rounded-xl bg-cyan-950/40 border border-cyan-500/40 flex items-start gap-3">
                ...
                <div className="text-xs font-mono">
                  <span className="text-cyan-300 font-bold">Xác nhận đơn hàng cẩn thận: </span>
                  <span className="text-slate-300">
                    Vui lòng kiểm tra đúng sản phẩm, số lượng và giá tiền bên dưới để đảm bảo bạn không bấm nhầm trước khi tiền được trừ tự động từ ví.
                  </span>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-slate-800 overflow-hidden bg-slate-950/80">
                <div className="p-3 bg-slate-900 border-b border-slate-800 text-xs font-mono font-bold text-slate-300 flex items-center justify-between">
                  <span>DANH SÁCH SẢN PHẨM MUA LẺ ({totalItemCount} MÓN)</span>
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 text-[11px] text-slate-400 mt-0.5">
                              ...
                              <span>•</span>
                              <span>Đơn giá: {formatCurrency(item.product.retailPrice, currentUser.currency)}</span>
                              <span>•</span>
                              <span>Số lượng: <strong className="text-white">x{item.quantity}</strong></span>
                            </div>
                          </div>
                        </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  ...
                  >
                    ...
                    <div className="text-[10px] text-slate-400 font-mono mt-0.5">
                      {hasEnoughBalance ? '✓ Đủ số dư thanh toán' : '⚠️ Số dư ví chưa đủ'}
                    </div>
                  </button>
              ...
              {paymentMethod === 'wallet' && !hasEnoughBalance && (
                <div className="p-3.5 rounded-xl bg-amber-950/50 border border-amber-500/40 flex items-center justify-between gap-3 text-xs font-mono">
                  <div className="flex items-center gap-2 text-amber-300">
                    ...
                    <span>
                      Số dư ví hiện tại ({formatCurrency(currentUser.walletBalance, currentUser.currency)}) không đủ thanh toán ({formatCurrency(finalTotal, currentUser.currency)}).
                    </span>
                  </div>
                  {onOpenDeposit && (
                    <button
              ...
              <div className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800 text-xs font-mono space-y-1.5">
                <div className="flex items-center justify-between text-slate-400">
                  <span>Hình thức nhận key:</span>
                  <span className="text-cyan-300 font-bold">⚡ Bung trực tiếp & lưu vào Kho Key (Vault)</span>
                </div>
                <div className="flex items-center justify-between text-slate-400">
                ...
                <div className="flex items-center justify-between text-slate-400">
                  <span>Bảo hiểm Escrow:</span>
                  <span className="text-emerald-400 font-bold">Bảo hành 1 đổi 1 trong 30 ngày</span>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-gradient-to-b from-[#0e1628] to-[#0a0f1d] border border-cyan-500/40 space-y-2.5 text-xs font-mono">
                ...
                <label className="flex items-start gap-2.5 pt-2 border-t border-slate-800/80 cursor-pointer select-none text-slate-300 hover:text-white">
                  <input
                    type="checkbox"
                    checked={hasConfirmedAgreement}
                    onChange={(e) => setHasConfirmedAgreement(e.target.checked)}
                    className="w-4 h-4 rounded border-slate-700 bg-slate-800 text-cyan-500 focus:ring-cyan-400 mt-0.5 cursor-pointer"
                  />
                  <span className="text-[11px] leading-relaxed">
                    Tôi đã kiểm tra kỹ danh sách sản phẩm trên, xác nhận không bấm nhầm và đồng ý thanh toán số tiền <strong className="text-cyan-300">{formatCurrency(finalTotal, currentUser.currency)}</strong>.
                  </span>
                </label>
              </div>
                ...
                >
                  {isProcessing ? (
                    <>
                      ...
                      <span>ĐANG XỬ LÝ KHẤU TRỪ VÍ & BUNG KEY...</span>
                    </>
                  ) : (
                    <>
                      <Lock className="w-4 h-4" />
                      <span>XÁC NHẬN MUA & BUNG KEY NGAY ({formatCurrency(finalTotal, currentUser.currency)})</span>
                    </>
                  )}
                </button>
              </div>
