          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold font-mono text-white flex items-center gap-2">
                <span>TRUNG TÂM BẢO HÀNH & KHIẾU NẠI ESCROW (TICKETS)</span>
              ...
              <p className="text-xs text-slate-400 font-mono mt-0.5">
                Kiểm tra tự động và đổi mã bản quyền 1:1 tức thì theo chuẩn ShopClone7 / CyberPool
              </p>
            </div>
          </div>
          ...
          <div className="md:col-span-4 border-r border-slate-800/80 bg-[#080b12] flex flex-col overflow-hidden">
            <div className="p-3 border-b border-slate-800 flex items-center justify-between">
              ...
              <button
                ...
                className="px-2.5 py-1 rounded bg-cyan-500 hover:bg-cyan-400 text-black font-mono font-bold text-[11px] flex items-center gap-1 transition-all"
              >
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`px-1.5 py-0.5 rounded text-[9px] font-mono uppercase ${
                        ...
                        {ticket.status === 'auto_replaced' ? 'ĐÃ ĐỔI KEY MỚI' : ticket.status}
                      </span>
                    </div>
              ...
              <form onSubmit={handleCreateSubmit} className="p-6 space-y-4 flex-1 overflow-y-auto">
                <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                  <h3 className="text-sm font-mono font-bold text-white flex items-center gap-2">
                    ...
                    <span>TẠO YÊU CẦU BẢO HÀNH & KHIẾU NẠI MỚI</span>
                  </h3>
                  <button
                    type="button"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-mono text-slate-300">Phân loại sự cố</label>
                  <select
                    value={newCategory}
                  ...
                  >
                    <option value="Key Issue">Lỗi Key / Code (Không kích hoạt được / Bị trùng)</option>
                    <option value="Top-Up Delay">Nạp Game Chậm / Chưa nhận được vật phẩm</option>
                    <option value="Escrow Refund">Yêu cầu mở khóa hoàn tiền cọc Escrow</option>
                    <option value="General Support">Hỗ trợ kỹ thuật & kết nối API</option>
                  </select>
                </div>

                <div className="space-y-1">
                  <label className="text-xs font-mono text-slate-300">Tiêu đề yêu cầu</label>
                  <input
                    type="text"
                    value={newSubject}
                    onChange={(e) => setNewSubject(e.target.value)}
                    placeholder="VD: Mã key Steam vừa nhận báo đã được sử dụng"
                    className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 font-mono text-xs text-white focus:outline-none focus:border-cyan-400"
                    required
                  />
                </div>
                ...
                <div className="space-y-1">
                  <label className="text-xs font-mono text-slate-300">Nội dung chi tiết & Mã đơn hàng</label>
                  <textarea
                    rows={4}
                    value={newMessage}
                    onChange={(e) => setNewMessage(e.target.value)}
                    placeholder="Mô tả cụ thể thông báo lỗi gặp phải để robot tự động kiểm duyệt kho..."
                    className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 font-mono text-xs text-white focus:outline-none focus:border-cyan-400"
                    required
                  />
                </div>
                ...
                <button
                  type="submit"
                  className="w-full py-3 rounded-lg bg-cyan-500 hover:bg-cyan-400 text-black font-mono font-bold text-xs uppercase tracking-wider transition-all"
                >
                  Gửi Yêu Cầu & Kích Hoạt Robot Đổi Key
                </button>
              </form>
            ) : activeTicket ? (
              <div className="flex flex-col h-full overflow-hidden">
                          ...
                          {msg.attachmentKey && (
                            <div className="mt-3 p-3 rounded-lg bg-black/80 border border-emerald-500/50 space-y-2 text-left">
                              <div className="text-[10px] font-bold font-mono text-emerald-400 flex items-center gap-1.5 uppercase">
                                ...
                                <span>MÃ BẢN QUYỀN MỚI TỰ ĐỘNG BÙ ĐẮP:</span>
                              </div>
                              <div className="flex items-center justify-between gap-2 p-2 rounded bg-slate-950 border border-slate-800 font-mono text-xs text-white">
                                ...
                                <button
                                  ...
                                  className="p-1.5 rounded bg-slate-800 hover:bg-slate-700 text-cyan-400 shrink-0"
                                >
                ...
                <div className="p-3 border-t border-slate-800 bg-slate-900/80 flex items-center gap-2">
                  <input
                    ...
                    placeholder="Nhập câu trả lời hoặc gửi bổ sung thông tin..."
                  ...
                  <button
                    ...
                    className="p-2.5 rounded-lg bg-cyan-500 hover:bg-cyan-400 text-black font-bold transition-colors"
                  >
