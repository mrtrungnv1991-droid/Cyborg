...
export const FanMenuModal: React.FC<FanMenuModalProps> = ({
  ...
  const menuItems: FanMenuItem[] = [
    {
      id: 'topup',
      title: 'Nạp Game Tự Động (API)',
      subtitle: '121 Game • 1.702 Gói nạp tức thì 3-5s',
      category: 'gaming',
    ...
    {
      id: 'flash_sale',
      title: 'Cyber Flash Sale',
      subtitle: 'Deal chớp nhoáng giảm giá đến 81%',
      category: 'gaming',
    ...
    {
      id: 'lucky_wheel',
      title: 'Vòng Quay May Mắn (Jackpot)',
      subtitle: 'Trúng Giftcard, Gems & tiền mặt live',
      category: 'tools',
    ...
    },
    {
      ...
      subtitle: 'Viettel, Vina, Mobi qua API tự động',
      category: 'finance',
    ...
    {
      id: 'deposit_hub',
      title: 'Nạp Tiền QR Tự Động',
      subtitle: 'MBBank, Vietcombank, Momo, USDT',
      category: 'finance',
      icon: <CreditCard className="w-5 h-5" />,
      badge: 'TỰ ĐỘNG 3S',
    ...
    {
      id: 'key_vault',
      title: 'Kho Key & GiftUp Vault',
      subtitle: 'Xem & quản lý tất cả key, tài khoản đã mua',
      category: 'gaming',
    ...
    {
      id: 'create_pool',
      title: 'Mở Nhóm Gom Đơn Mới',
      subtitle: 'Tạo pool mua chung nhận giá sỉ',
      category: 'gaming',
    ...
    {
      id: 'order_lookup',
      title: 'Tra Cứu Đơn Hàng Nhanh',
      subtitle: 'Kiểm tra trạng thái đơn qua mã hoặc email',
      category: 'tools',
    ...
    {
      id: 'ledger',
      title: 'Sao Kê & Biến Động Số Dư',
      subtitle: 'Lịch sử dòng tiền chi tiết thời gian thực',
      category: 'finance',
    ...
    {
      id: 'key_tools',
      title: 'Công Cụ Key & Token (Tools)',
      subtitle: 'Check định dạng key, tách token, test API',
      category: 'tools',
    ...
    {
      id: 'tickets',
      title: 'Bảo Hành 1:1 & Khiếu Nại',
      subtitle: 'Gửi yêu cầu hỗ trợ, đổi key lỗi siêu tốc',
      category: 'support',
    ...
    {
      id: 'affiliate',
      title: 'Đại Lý Sỉ & CTV (Affiliate)',
      subtitle: 'Chiết khấu hoa hồng cấp đại lý, API cấp lại',
      category: 'tools',
    ...
    {
      id: 'suppliers',
      title: 'Nhà Cung Cấp & Kết Nối API',
      subtitle: 'Quản lý cổng tổng kho, kết nối nguồn hàng',
      category: 'tools',
    ...
    {
      id: 'escrow',
      title: 'Hướng Dẫn Bảo Lãnh Escrow',
      subtitle: 'Cơ chế giữ tiền thông minh an toàn 100%',
      category: 'support',
    ...
    },
    {
      ...
      subtitle: 'Quản trị viên: Tùy chỉnh game, tiers, tồn kho, thành viên',
      category: 'admin',
        ...
        >
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="text-base sm:text-lg font-black font-mono text-white tracking-wide uppercase flex items-center gap-1.5">
                    <span>THƯ MỤC TẤT CẢ TIỆN ÍCH</span>
                    <span className="text-cyan-400">// XOÈ QUẠT MENU</span>
                  </h2>
                  <span className="px-2 py-0.5 rounded text-[10px] font-mono bg-cyan-950 text-cyan-300 border border-cyan-500/40 font-bold hidden sm:inline">
                    {menuItems.length} TÍNH NĂNG
                  </span>
                </div>
                <p className="text-xs text-slate-400 font-mono">Truy cập siêu tốc mọi dịch vụ, minigame, ví số dư và công cụ hệ thống</p>
              </div>
            </div>
          ...
          <div className="p-3 sm:p-4 bg-slate-950/60 border-b border-slate-800/80 space-y-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Tìm kiếm nhanh tính năng, nạp game, thẻ cào, ví tiền, admin..."
          ...
          <div className="p-3 sm:p-5 overflow-y-auto flex-1 space-y-4">
              ...
              <div className="relative py-4 px-2 hidden md:block overflow-hidden rounded-xl bg-gradient-to-b from-[#0b0f19] to-black border border-cyan-500/20">
                <div className="text-center mb-3">
                  <span className="text-[11px] font-mono text-cyan-400 uppercase tracking-widest bg-cyan-950/60 px-3 py-1 rounded-full border border-cyan-500/30">
                    ✨ Cụm Xòe Quạt Tiện Ích Đỉnh Cao (Nhấp Để Mở Ngay)
                  </span>
                </div>
            ...
            {filteredItems.length === 0 && (
              <div className="p-8 text-center rounded-xl bg-slate-900/40 border border-dashed border-slate-800 space-y-2">
                ...
                <div className="text-xs font-mono text-slate-300">Không tìm thấy tiện ích phù hợp với "{searchQuery}"</div>
                <button
                  onClick={() => setSearchQuery('')}
          ...
          <div className="p-3 sm:p-4 border-t border-slate-800 bg-[#07090e] flex flex-col sm:flex-row items-center justify-between gap-2 text-xs font-mono text-slate-400">
            <div className="flex items-center gap-2">
              ...
              <span>Số dư: <strong className="text-emerald-400">{user.walletBalance.toLocaleString('vi-VN')} {user.currency}</strong></span>
              <span>• Khách hàng: <strong className="text-cyan-300">{user.name}</strong></span>
            </div>

            <div className="flex items-center gap-2">
              <button
                onClick={() => { onClose(); onOpenDepositHub(); }}
                className="px-3 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-black font-bold uppercase transition-all text-xs"
              >
                + Nạp Tiền
              </button>
              <button
                onClick={onClose}
                className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-white transition-all text-xs"
              >
                Đóng
              </button>
            </div>
