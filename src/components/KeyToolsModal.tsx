          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold font-mono text-white tracking-wide">
                BỘ CÔNG CỤ TIỆN ÍCH KEY & CHECKLIVE GAME OS
              </h2>
              <p className="text-xs text-slate-400 font-mono mt-0.5">
                Tool lọc format danh sách tài khoản, kiểm tra định dạng Steam Key & Ping UID
              </p>
            </div>
          </div>
          ...
          {activeTab === 'formatter' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <label className="text-xs font-mono text-slate-300">Nhập danh sách (mỗi dòng 1 tài khoản/key):</label>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-mono text-slate-400">Định dạng đích:</span>
                  <select
                    value={outputFormat}
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-black font-mono font-bold text-xs uppercase"
              >
                Xử Lý & Chuyển Đổi Format Ngay
              </button>

              {formattedOutput && (
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-xs font-mono text-slate-400">
                    <span>Kết quả xuất:</span>
                    <button
          ...
          {activeTab === 'steam_validator' && (
            <div className="space-y-4">
              <label className="text-xs font-mono text-slate-300">
                Nhập danh sách Steam Key (dạng XXXXX-XXXXX-XXXXX):
              </label>
              <textarea
                rows={4}
                value={steamInput}
                ...
                <div className="space-y-2">
                  {steamResults.map((r, i) => (
                    ...
                    >
                      ...
                      >
                        {r.valid ? 'Hợp lệ (Valid Format)' : 'Sai định dạng'}
                      </span>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === 'uid_checker' && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-mono text-slate-300">Chọn Game:</label>
                  <select
                    value={selectedGame}
                ...
                <div>
                  <label className="text-xs font-mono text-slate-300">Nhập UID / OpenID:</label>
                  <input
                    type="text"
                    value={uidInput}
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-black font-mono font-bold text-xs uppercase flex items-center justify-center gap-2"
              >
                {isCheckingUid ? (
                  <>
                    <RefreshCw className="w-4 h-4 animate-spin" />
                    <span>Đang Ping Server API...</span>
                  </>
                ) : (
                  <span>Tra Cứu Nhân Vật Ngay</span>
                )}
              </button>
              ...
              {uidResult && (
                <div className="p-4 rounded-xl bg-emerald-950/30 border border-emerald-500/40 text-xs font-mono space-y-2">
                  <div className="text-emerald-400 font-bold flex items-center gap-2">
                    ...
                    <span>TÌM THẤY TÀI KHOẢN HỢP LỆ TRÊN MÁY CHỦ</span>
                  </div>
                  <div className="text-slate-300">Tên Nhân Vật: <strong className="text-white">{uidResult.nickname}</strong></div>
                  <div className="text-slate-300">Máy chủ: <strong className="text-white">{uidResult.server}</strong></div>
                  <div className="text-slate-300">Trạng thái: <span className="text-cyan-300">{uidResult.status}</span></div>
                </div>
              )}
            </div>
          )}
