...
export const CartModal: React.FC<CartModalProps> = ({
  ...
  const handleApplyCoupon = (e: React.FormEvent) => {
    ...
    if (clean === 'CYBER2026') {
      ...
      setCouponError('Mã giảm giá không tồn tại hoặc đã hết lượt dùng');
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-mono mt-0.5">
                Kiểm tra sản phẩm số, số lượng & mã giảm giá trước khi xác nhận đặt mua
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {cartItems.length > 0 && (
              <button
                ...
                title="Xoá tất cả sản phẩm trong giỏ"
              >
          ...
          {cartItems.length === 0 ? (
            <div className="py-12 px-4 text-center">
              ...
              <p className="text-xs text-slate-400 max-w-sm mx-auto mb-6 font-mono">
                Chưa có sản phẩm mua lẻ nào được thêm vào giỏ. Hãy dạo chợ để chọn tài khoản số, key game hoặc key bản quyền nhé!
              </p>
              <button
                type="button"
                onClick={() => {
              ...
              >
                <span>DẠO CHỢ KHÁM PHÁ SẢN PHẨM</span>
            ...
            <>
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-slate-900/60 border border-slate-800 text-xs font-mono">
                ...
                <div className="text-slate-400 text-[11px]">
                  Đã chọn: <span className="text-cyan-400 font-bold">{selectedCount}</span> món
                </div>
              </div>
              ...
              <div className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800 space-y-2">
                <div className="flex items-center justify-between text-xs font-mono">
                  <span className="flex items-center gap-1.5 text-slate-300 font-bold">
                    ...
                    <span>Mã Ưu Đãi / Voucher Giảm Giá</span>
                  </span>
                  <span className="text-[10px] text-slate-400">
                    Gợi ý: <span className="text-cyan-400 font-bold">CYBER2026</span> (-10%), <span className="text-cyan-400 font-bold">VIP10</span>
                  </span>
                </div>
                ...
                {appliedCoupon ? (
                  ...
                  <form onSubmit={handleApplyCoupon} className="flex gap-2">
                    <input
                      ...
                      placeholder="Nhập mã voucher giảm giá (vd: CYBER2026)..."
              ...
              <div className="p-4 rounded-xl bg-gradient-to-b from-[#0e1628] to-[#0a0f1d] border border-cyan-500/30 space-y-2 text-xs font-mono">
                <div className="flex justify-between text-slate-400">
                  <span>Tổng tiền hàng ({selectedCount} sản phẩm đã chọn):</span>
                ...
                {bulkDiscountPercent > 0 && (
                  <div className="flex justify-between text-emerald-400">
                    <span className="flex items-center gap-1">
                      ...
                      <span>Chiết khấu mua nhiều (-{bulkDiscountPercent}%):</span>
                    </span>
                ...
                <div className="flex justify-between text-slate-400">
                  <span>Phí bảo lãnh ký quỹ Escrow:</span>
                  <span className="text-emerald-400 font-bold">Miễn phí (0đ)</span>
                </div>

                <div className="pt-2 border-t border-slate-800 flex justify-between items-baseline">
                  <span className="text-sm font-bold text-white">Tổng thanh toán dự kiến:</span>
                  <div className="text-right">
                    <span className="text-lg font-black text-cyan-400 font-mono">
                      {formatCurrency(finalTotal, currentUser.currency)}
                    </span>
                    <div className="text-[10px] text-slate-400">Đã bao gồm VAT & Bung Key Tức Thì</div>
                  </div>
                </div>
              </div>
            </>
        ...
        {cartItems.length > 0 && (
          <div className="p-4 sm:p-5 border-t border-slate-800 bg-[#070b14] flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0">
            <div className="text-xs font-mono text-slate-400 text-center sm:text-left">
              <span>Đã chọn <strong className="text-cyan-400">{selectedCount}</strong>/{totalItemCount} món</span>
              <span className="mx-2">•</span>
              <span>Tổng: <strong className="text-white font-bold">{formatCurrency(finalTotal, currentUser.currency)}</strong></span>
            </div>
