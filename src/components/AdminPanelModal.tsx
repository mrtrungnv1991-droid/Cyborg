...
export const AdminPanelModal: React.FC<AdminPanelModalProps> = ({
  ...
  const navTabs: { id: AdminTabType; label: string; icon: React.ComponentType<{ className?: string }>; badge?: string | number }[] = [
    ...
    { id: 'vouchers', label: 'Mã Giảm Giá (Vouchers)', icon: Tag, badge: vouchers?.length },
    { id: 'banking', label: 'Duyệt Nạp Tiền & Banking', icon: CreditCard, badge: topupInvoices?.filter(i => i.status === 'pending').length },
    { id: 'members', label: 'Thành Viên & Ví', icon: Users, badge: members.length },
    { id: 'livechat', label: 'Live CSKH 24/7', icon: MessageSquare, badge: chatSessions.length },
    { id: 'escrow_orders', label: 'Đơn Hàng & Escrow', icon: Lock, badge: orders.length },
    { id: 'games', label: 'Kho Top-up Games', icon: Gamepad2, badge: '121 Game' },
    { id: 'tickets', label: 'Khiếu Nại & Bảo Hành', icon: LifeBuoy, badge: tickets.filter(t => t.status === 'open').length },
    { id: 'roles', label: 'Phân Quyền & Sub-Admin', icon: ShieldCheck },
    { id: 'security_ip', label: 'Tường Lửa & IP WAF', icon: ShieldAlert },
    { id: 'automation_cron', label: 'Cron Jobs & Auto Sync', icon: Clock },
    { id: 'logs', label: 'Nhật Ký Hệ Thống Logs', icon: FileText },
    { id: 'suppliers', label: 'Nhà Cung Cấp APIs', icon: Server, badge: suppliers.length },
    { id: 'affiliate', label: 'Affiliate & Hoa Hồng', icon: Share2 },
    { id: 'giftup_admin', label: 'Kho GiftUp Cards', icon: Gift },
    { id: 'settings', label: 'Cài Đặt Hệ Thống CMS', icon: Settings },
    { id: 'audit_security', label: 'Bảo Mật Anti-DDoS', icon: ShieldAlert },
    { id: 'database_schema', label: '64 Bảng Database SQL', icon: Database, badge: '64' }
  ];
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-sm sm:text-base font-extrabold text-white uppercase tracking-wider">
                  SHOPCLONE7 // ROOT ADMIN DISPATCH CONTROL PANEL (ADCP)
                </h2>
              ...
              <p className="text-xs text-slate-400 font-medium">
                Toàn quyền giám sát nạp tự động, điều phối kho key, đối soát nạp game & cấu hình hệ thống
              </p>
            </div>
          </div>
