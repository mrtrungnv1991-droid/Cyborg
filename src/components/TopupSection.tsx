    ...
    <section className="space-y-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-base sm:text-lg font-bold font-mono text-white tracking-wide flex items-center gap-2 flex-wrap">
              <span>CỔNG NẠP GAME TỰ ĐỘNG & GOM GÓI SỈ</span>
            ...
            <p className="text-xs text-slate-400 font-mono">
              Kết nối Midasbuy, SmileOne, TheSieuRe API • Nạp trực tiếp UID game an toàn qua Escrow
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 justify-between lg:justify-end flex-wrap">
          ...
          <div className="flex items-center bg-black/60 p-1 rounded-lg border border-slate-800 text-xs font-mono">
            <button
              ...
              title="Chế độ cuộn ngang mượt mà"
            >
              <SlidersHorizontal className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">Cuộn Ngang</span>
            </button>
            <button
              ...
              title="Chế độ lưới toàn màn hình"
            >
              <LayoutGrid className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">Lưới Grid</span>
            </button>
          </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2 items-center justify-between">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            placeholder={`Tìm kiếm trong ${games.length} tựa game...`}
      ...
      {filteredGames.length === 0 ? (
        <div className="p-8 sm:p-12 text-center rounded-xl bg-slate-900/40 border border-dashed border-slate-800 space-y-2">
          ...
          <div className="text-xs font-mono text-slate-400">Không tìm thấy tựa game nào phù hợp với bộ lọc</div>
        </div>
      ) : viewMode === 'carousel' ? (
        ...
        <div className="relative group/carousel">
            ...
            {filteredGames.map(game => (
              ...
              >
                ...
                <div className="mt-2 sm:mt-3 pt-1.5 sm:pt-2 border-t border-slate-800/80 flex items-center justify-between text-[11px] sm:text-xs font-mono">
                  <div>
                    <div className="text-[8px] sm:text-[10px] text-slate-500">Từ</div>
                    <div className="font-bold text-cyan-400 text-xs sm:text-sm">
                      {game.tiers[0] ? formatCurrency(game.tiers[0].groupPrice, currency) : 'Liên hệ'}
                    </div>
                  </div>
          ...
          <div className="mt-2 flex items-center justify-between text-[10px] sm:text-[11px] font-mono text-slate-400">
            <div className="flex items-center gap-1.5">
              ...
              <span>Cuộn ngang để xem tất cả {filteredGames.length} tựa game</span>
            </div>

            <div className="hidden sm:flex items-center gap-2">
              <span className="text-[10px] text-slate-500">Tiến độ cuộn:</span>
        ...
        <div className="space-y-4">
          ...
          {/* Expand and Collapse (Thu gọn) Dual-Action Buttons */}
          <div className="flex items-center justify-center gap-3 pt-2">
            {filteredGames.length > displayCount && (
              ...
              >
                ...
                <span>Xem Thêm Game (+18) • Đang hiển thị {displayCount}/{filteredGames.length}</span>
              </button>
            )}
            ...
            {displayCount > 18 && (
              ...
              >
                ...
                <span>Thu Gọn Lại (Về 18 Game)</span>
              </button>
            )}
          </div>
        </div>
