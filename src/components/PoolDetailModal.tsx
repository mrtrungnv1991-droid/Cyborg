...
export const PoolDetailModal: React.FC<PoolDetailModalProps> = ({
  ...
  const getRatingLabel = (stars: number) => {
    switch (stars) {
      ...
      case 4: return 'Hài lòng & Uy tín';
      case 5: return 'Tuyệt vời, 5 sao chất lượng!';
      default: return '';
    }
  };
      ...
      <div className="relative w-full max-w-3xl rounded-xl bg-[#0c0f17] border border-slate-700 shadow-2xl overflow-hidden my-8">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-xs font-mono text-amber-400 flex items-center gap-1">
                  <Clock className="w-3 h-3" /> Hết hạn sau: {pool.expiresAt}
                </span>
                {/* Rating Badge in Header */}
                <button
                  onClick={() => setActiveTab('reviews')}
                  className="text-xs font-mono px-2 py-0.5 rounded bg-amber-950/60 text-amber-300 border border-amber-500/40 flex items-center gap-1 hover:bg-amber-900/60 transition-colors"
                >
                  <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
                  <span className="font-bold">{averageRating}</span>
                  <span className="text-slate-400">({totalReviewCount} đánh giá)</span>
                </button>
              </div>
        ...
        <div className="flex border-b border-slate-800 bg-[#090c14] px-4 sm:px-6">
          ...
          >
            ...
            <span>Tiến Độ Gom Đơn ({pool.filledSlots}/{pool.targetSlots})</span>
          </button>
          ...
          >
            ...
            <span>Đánh Giá Sao & Nhận Xét ({totalReviewCount})</span>
          </button>
        </div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 rounded-lg bg-slate-900/80 border border-slate-800">
                  <div className="text-xs font-mono text-slate-400">Giá gom đơn (Mua chung)</div>
                  <div className="text-2xl font-black font-mono text-cyan-400 mt-1">
                    {formatCurrency(pool.pricePerSlot, user.currency)}
                  </div>
                  <div className="text-[11px] font-mono text-emerald-400 mt-0.5">
                    Tiết kiệm {pool.savingsPercent}% so với giá lẻ
                  </div>
                </div>

                <div className="p-4 rounded-lg bg-slate-900/80 border border-slate-800">
                  <div className="text-xs font-mono text-slate-400">Tiến độ gom slot</div>
                  <div className="text-2xl font-black font-mono text-amber-400 mt-1">
                    {pool.filledSlots} / {pool.targetSlots} Slots
                  </div>
                  <div className="text-[11px] font-mono text-slate-300 mt-0.5">
                    {remainingSlots > 0 ? `Cần thêm ${remainingSlots} người để bung key` : 'Đã đủ số lượng!'}
                  </div>
                </div>

                <div className="p-4 rounded-lg bg-slate-900/80 border border-slate-800">
                  <div className="text-xs font-mono text-slate-400">Bảo lãnh giao dịch</div>
                  <div className="text-base font-bold font-mono text-emerald-400 mt-1 flex items-center gap-1">
                    <ShieldCheck className="w-4 h-4" /> Ví Escrow Tự Động
                  </div>
                  <div className="text-[11px] font-mono text-slate-400 mt-0.5">
                    100% Hoàn tiền nếu nhóm không đủ
                  </div>
                </div>
              </div>
                ...
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-bold font-mono text-white">
                      Danh Sách Thành Viên Đặt Slot Trong Nhóm ({pool.filledSlots}/{pool.targetSlots})
                    </h3>
                  </div>
                  ...
                  <button
                    ...
                    title="Mô phỏng 1 người mua khác tham gia slot"
                  >
                    <UserPlus className="w-3.5 h-3.5" />
                    <span>+ Mô Phỏng Khách Vào Gom</span>
                  </button>
                </div>
                        ...
                        {isOccupied ? (
                          ...
                          <>
                            ...
                            <div className="flex-1">
                              <div className="text-xs font-mono text-slate-500 font-medium">Slot Trống</div>
                              <div className="text-[10px] font-mono text-cyan-400">Đang chờ bạn...</div>
                            </div>
                          </>
                        )}
                      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-3.5 rounded-xl bg-slate-950/90 border border-cyan-500/30 text-xs">
                <div className="space-y-1 border-b md:border-b-0 md:border-r border-slate-800 pb-2 md:pb-0 md:pr-3">
                  <div className="text-cyan-400 font-bold uppercase flex items-center gap-1.5 text-[11px]">
                    ...
                    <span>📥 ĐẦU VÀO CỦA BẠN (INPUT)</span>
                  </div>
                  <p className="text-[11px] text-slate-300 leading-relaxed font-sans">
                    Đặt cọc <strong>{formatCurrency(pool.pricePerSlot, user.currency)}</strong> để giữ 1 slot mua sỉ. Tiền cọc được khóa an toàn tuyệt đối 100% trong két Escrow bảo lãnh.
                  </p>
                </div>
                ...
                <div className="space-y-1 md:pl-3">
                  <div className="text-emerald-400 font-bold uppercase flex items-center gap-1.5 text-[11px]">
                    ...
                    <span>📤 ĐẦU RA BẠN NHẬN ĐƯỢC (OUTPUT)</span>
                  </div>
                  <p className="text-[11px] text-slate-300 leading-relaxed font-sans">
                    • <strong>Đủ {pool.targetSlots} slot:</strong> Tự động bung mã Key vào <strong>Kho Key Cá Nhân</strong> của bạn tức thì.
                    <br />• <strong>Không đủ người:</strong> Tự động hoàn tiền 100% về ví ngay lập tức.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-mono">
                <div className="p-3.5 rounded-lg bg-slate-900/60 border border-slate-800 space-y-2">
                  <div className="text-cyan-400 font-bold flex items-center gap-1.5">
                    ...
                    <span>Phương Thức Nhận Key / Tài Khoản</span>
                  </div>
                  <p className="text-slate-300 leading-relaxed font-sans text-xs">
                    {product.deliveryEstimate}. Khi pool đủ {pool.targetSlots} người, hệ thống tự động bung mã CD-Key hoặc thẻ GiftUp trực tiếp vào <strong>Kho Key Của Bạn</strong> kèm hướng dẫn kích hoạt.
                  </p>
                </div>
                ...
                <div className="p-3.5 rounded-lg bg-slate-900/60 border border-slate-800 space-y-2">
                  <div className="text-emerald-400 font-bold flex items-center gap-1.5">
                    ...
                    <span>Hợp Đồng Bảo Vệ Tiền Nạp (Escrow)</span>
                  </div>
                  <p className="text-slate-300 leading-relaxed font-sans text-xs">
                    Số tiền {formatCurrency(pool.pricePerSlot, user.currency)} được tạm khóa an toàn trong ví Escrow. Người bán chỉ nhận được thanh toán khi bạn đã kích hoạt thành công. Nếu pool hết hạn mà chưa đủ slot, tiền tự động hoàn trả 100%.
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 sm:p-6 rounded-xl bg-gradient-to-r from-[#111625] via-slate-900 to-[#111625] border border-amber-500/30 flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  <div className="text-center sm:text-left">
                    ...
                    <div className="text-xs font-mono text-slate-400 mt-1">
                      Dựa trên {totalReviewCount} lượt đánh giá thực tế
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 rounded-lg bg-emerald-950/40 border border-emerald-500/30 text-center sm:text-right shrink-0">
                  ...
                  <div className="text-[10px] text-slate-400 font-mono mt-0.5">
                    Đánh giá từ người mua hoàn tất
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 sm:p-5 rounded-xl bg-slate-900/90 border border-slate-700/80 space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold font-mono text-white flex items-center gap-2">
                    ...
                    <span>Gửi Đánh Giá Của Bạn Cho Sản Phẩm Này</span>
                  </h3>
                ...
                <form onSubmit={handleSubmitRating} className="space-y-3">
                  ...
                  <div className="p-3 rounded-lg bg-slate-950 border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs font-mono text-slate-400 mr-2">Chọn số sao:</span>
                      {[1, 2, 3, 4, 5].map(star => {
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="text-[10px] font-mono text-slate-500">Mẫu nhanh:</span>
                  ...
                  <div className="relative">
                    <textarea
                      ...
                      placeholder="Chia sẻ trải nghiệm kích hoạt key, tốc độ bung slot hoặc nhận xét tài khoản của bạn..."
                      rows={2}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-[11px] font-mono text-slate-400 flex items-center gap-1.5">
                      ...
                      />
                      <span>Đánh giá dưới tên: <strong className="text-cyan-400">{user.name}</strong></span>
                    </div>
                    ...
                    >
                      ...
                      <span>Gửi Đánh Giá ({userRating}⭐)</span>
                    </button>
                  </div>
                </form>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-bold font-mono text-slate-400 uppercase tracking-wider">
                    Nhận Xét Từ Khách Hàng ({reviewsList.length})
                  </h3>
                </div>
                ...
                {reviewsList.length === 0 ? (
                  <div className="p-6 rounded-xl bg-slate-950/60 border border-dashed border-slate-800 text-center space-y-2">
                    ...
                    <p className="text-xs font-mono text-slate-400">
                      Chưa có nhận xét nào được hiển thị. Hãy là người đầu tiên đánh giá sản phẩm này!
                    </p>
                  </div>
                ) : (
                              ...
                              <div className="flex items-center gap-1.5">
                                ...
                                {review.verifiedPurchase && (
                                  <span className="px-1.5 py-0.2 rounded text-[9px] font-mono bg-emerald-950 text-emerald-400 border border-emerald-500/30 flex items-center gap-0.5">
                                    <CheckCircle2 className="w-2.5 h-2.5" /> Đã mua
                                  </span>
                                )}
                              </div>
        ...
        <div className="p-4 sm:p-6 border-t border-slate-800 bg-[#0a0d14] flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <div className="text-xs font-mono text-slate-400">
              Tổng Chi Phí ({selectedSlotCount} Slot):
            </div>
            <div className="text-xl sm:text-2xl font-black font-mono text-cyan-400">
              {formatCurrency(totalCost, user.currency)}
            </div>
            <div className="text-[11px] font-mono text-slate-500">
              Số dư khả dụng: <span className="text-white font-bold">{formatCurrency(user.walletBalance, user.currency)}</span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
            ...
            {!hasEnoughBalance ? (
              ...
              >
                {isJoined ? (
                  <span>Bạn Đã Có Slot Trong Nhóm Này</span>
                ) : isFull ? (
                  <span>Nhóm Đã Đủ Slot</span>
                ) : (
                  <>
                    <span>Khóa Slot Gom Đơn Ngay</span>
