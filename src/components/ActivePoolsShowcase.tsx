...
import { Product, GroupPool, CurrencyCode, LanguageCode } from '../types';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            <h2 className="text-lg sm:text-xl font-bold font-mono text-white tracking-wide flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded text-[11px] bg-cyan-950 text-cyan-300 border border-cyan-500/40 font-mono">
                {displayedProducts.length} {t('showcase_items')}
              </span>
            </h2>
          </div>
          <p className="text-xs text-slate-400 font-mono flex items-center gap-2">
            <span className="inline-block w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
            <span>{t('showcase_escrow_note')}</span>
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="hidden lg:flex items-center gap-1 bg-slate-900/90 p-1 rounded-lg border border-slate-800 text-xs font-mono">
            ...
            >
              {t('showcase_tab_all')} ({products.length})
            </button>
            <button
          ...
          <div className="flex items-center bg-black/60 p-1 rounded-lg border border-slate-800 text-xs font-mono">
            <button
              ...
              title={t('showcase_scroll_mode')}
            >
              <SlidersHorizontal className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">{t('showcase_scroll_mode')}</span>
            </button>
            <button
              ...
              title={t('showcase_grid_mode')}
            >
              <LayoutGrid className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">{t('showcase_grid_mode')}</span>
            </button>
          </div>
      ...
      {displayedProducts.length === 0 ? (
        <div className="p-8 sm:p-12 text-center rounded-2xl bg-slate-900/40 border border-dashed border-slate-800 space-y-3">
          ...
          <div className="text-xs sm:text-sm font-mono text-slate-300">Không có nhóm gom nào trong danh mục này</div>
          <button
            onClick={() => setFilterType('all')}
            className="px-3.5 py-1.5 rounded-lg bg-cyan-500 text-black font-mono font-bold text-xs"
          >
            Xem Tất Cả Nhóm Gom
          </button>
        </div>
        ...
        <div className="relative group/carousel">
          ...
          <div className="mt-2 sm:mt-3 flex items-center justify-between text-[10px] sm:text-[11px] font-mono text-slate-400">
            <div className="flex items-center gap-1.5">
              ...
              <span>
                Vuốt ngang để xem tất cả {displayedProducts.length} nhóm gom đang mở
              </span>
            </div>
