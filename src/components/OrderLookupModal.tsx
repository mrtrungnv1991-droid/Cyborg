    ...
    if (match) {
      ...
      if (cleanQuery.includes('tx') || cleanQuery.includes('pool') || cleanQuery.length > 4) {
        setFoundOrder({
          ...
          productTitle: 'ChatGPT Plus 1 Tháng (Gói Gom Nhóm 5)',
          platform: 'OpenAI',
          type: 'group_buy',
          pricePaid: 155000,
          status: 'fulfilled',
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold font-mono text-white tracking-wide">
                TRA CỨU ĐƠN HÀNG & CHECKLIVE KEY TỰ ĐỘNG
              </h2>
              <p className="text-xs text-slate-400 font-mono mt-0.5">
                Kiểm tra tình trạng đơn gom, lấy lại Key bản quyền hoặc tiến độ nạp UID
              </p>
            </div>
          </div>
        ...
        <div className="p-4 sm:p-6 space-y-6 overflow-y-auto">
          <form onSubmit={handleSearch} className="space-y-3">
            <label className="text-xs font-mono font-bold text-slate-300">
              NHẬP MÃ GIAO DỊCH (TX-...), MÃ ĐƠN HÀNG HOẶC UID GAME
            </label>
            <div className="flex gap-2">
              <input
                ...
                placeholder="Ví dụ: TX-849204, ord-101, hoặc UID 849201948..."
                required
                ...
                <div className="p-4 rounded-xl bg-slate-900/70 border border-cyan-500/40 space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                    ...
                    <span className={`px-2.5 py-1 rounded text-xs font-mono font-bold uppercase border ${
                      ...
                      {foundOrder.status === 'fulfilled' ? 'Hoàn tất & Đã bung key' : 'Đang tạm giữ Escrow'}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs font-mono">
                    <div className="p-2.5 rounded bg-black/40 border border-slate-800">
                      <div className="text-slate-500 text-[10px]">Mã Giao Dịch TxID</div>
                      <div className="text-slate-200 font-bold mt-0.5">{foundOrder.txId}</div>
                    </div>

                    <div className="p-2.5 rounded bg-black/40 border border-slate-800">
                      <div className="text-slate-500 text-[10px]">Số Tiền Thanh Toán</div>
                    ...
                    <div className="p-3.5 rounded-lg bg-cyan-950/30 border border-cyan-500/40 space-y-2">
                      ...
                      <div className="flex items-center justify-between p-2.5 rounded bg-black/80 border border-slate-700">
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 rounded bg-cyan-500 hover:bg-cyan-400 text-black font-mono font-bold text-xs flex items-center gap-1"
                        >
                  ...
                  {foundOrder.topupDetails && (
                    <div className="p-3 rounded-lg bg-black/50 border border-slate-800 text-xs font-mono space-y-1">
                      <div className="text-slate-400">UID Nạp: <strong className="text-white">{foundOrder.topupDetails.uid}</strong></div>
                      ...
                      <div className="text-emerald-400 font-bold">Trạng thái nạp: Đã nạp thành công vào tài khoản game</div>
                    </div>
                  )}
                  ...
                  <div className="p-2.5 rounded bg-emerald-950/30 border border-emerald-500/30 flex items-center gap-2 text-xs font-mono text-emerald-300">
                    ...
                    <span>Đơn hàng được bảo hiểm 100% bởi Quỹ Escrow CyberPool Exchange.</span>
                  </div>
                </div>
              ) : (
                <div className="p-8 text-center rounded-xl bg-slate-900/30 border border-dashed border-slate-800 space-y-2">
                  <AlertCircle className="w-8 h-8 text-slate-500 mx-auto" />
                  <div className="text-xs font-mono text-slate-300">Không tìm thấy đơn hàng với thông tin trên</div>
                  <div className="text-[11px] font-mono text-slate-500">Vui lòng kiểm tra lại Mã TxID hoặc liên hệ hỗ trợ trực tuyến.</div>
                </div>
              )}
            </div>
          )}
