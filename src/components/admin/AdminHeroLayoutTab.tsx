...
import { SystemConfig, HeroCustomConfig, LaunchpadButtonConfig, UiLayoutConfig } from '../../types';
  ...
  return (
    <div className="space-y-6 text-slate-200">
      ...
      <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
        <div className="flex items-center justify-between border-b border-slate-800/80 pb-2.5">
          <div className="flex items-center gap-2">
            ...
            <span className="px-2 py-0.5 rounded text-[10px] bg-cyan-950 text-cyan-300 border border-cyan-500/30 font-mono">
              Khung: {heroConfig.containerMaxWidth}
            </span>
          </div>
        ...
        } ${heroConfig.heroBackground === 'cyber_grid' ? 'cyber-grid' : ''}`}>
          <div className={`${heroConfig.containerMaxWidth} mx-auto transition-all`}>
            {/* Header Content */}
            <div className={`flex ${
              heroConfig.contentAlignment === 'center' 
                ? 'flex-col items-center text-center' 
                : heroConfig.contentAlignment === 'left' 
                  ? 'flex-col items-start text-left' 
                  : 'flex-col lg:flex-row lg:items-end justify-between'
            } gap-4 mb-4`}>
              <div className="max-w-2xl">
                {heroConfig.badgeActive && (
                  <div className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded border text-[10px] sm:text-xs font-mono mb-2 shadow-sm ${getBadgeColorClass(heroConfig.badgeColor)}`}>
                ...
                <h1 className="text-xl sm:text-3xl lg:text-4xl font-black tracking-tight text-white uppercase font-mono leading-tight">
                  {heroConfig.mainHeadingLine1} <br />
                  <span className={`text-transparent bg-clip-text bg-gradient-to-r ${getGradientClass(heroConfig.mainHeadingGradient)}`}>
                    {heroConfig.mainHeadingLine2}
                  </span>
                </h1>
              ...
              {/* Metric Pods Preview */}
              {heroConfig.showTrustPods && (
                <div className="grid grid-cols-2 gap-2 font-mono text-xs shrink-0 w-full sm:w-auto">
                  {heroConfig.trustPod1.active && (
                    <div className="p-2.5 rounded-lg bg-slate-900/90 border border-slate-800">
                      <div className="text-slate-400 text-[9px] uppercase flex items-center gap-1">
                        <Zap className="w-3 h-3 text-cyan-400" /> {heroConfig.trustPod1.title}
                      </div>
                      <div className="text-xs sm:text-sm font-black text-cyan-400 mt-0.5">{heroConfig.trustPod1.value}</div>
                      <div className="text-[9px] text-emerald-400 mt-0.5">{heroConfig.trustPod1.sub}</div>
                    </div>
                  )}

                  {heroConfig.trustPod2.active && (
                    <div className="p-2.5 rounded-lg bg-slate-900/90 border border-slate-800">
                      <div className="text-slate-400 text-[9px] uppercase flex items-center gap-1">
                        <ShieldCheck className="w-3 h-3 text-emerald-400" /> {heroConfig.trustPod2.title}
                      </div>
                      <div className="text-xs sm:text-sm font-black text-emerald-400 mt-0.5">{heroConfig.trustPod2.value}</div>
                      <div className="text-[9px] text-slate-400 mt-0.5">{heroConfig.trustPod2.sub}</div>
                    </div>
                  )}
                </div>
              )}
            ...
            {/* Launchpad Preview */}
            {heroConfig.showLaunchpad && (
              <div className={`pt-2 border-t border-slate-800/60 ${
                heroConfig.launchpadLayout === 'wrap_grid'
                ...
                {heroConfig.launchpadButtons.filter(b => b.active).map(b => (
                  <div
                    key={b.id}
        ...
        <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-3 rounded-lg bg-slate-900/80 border border-slate-800 space-y-3">
              <div className="flex items-center justify-between">
                <label className="text-xs font-mono text-slate-300 font-bold">Thẻ Badge Thông Báo Nổi Bật</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-xs font-mono text-slate-300">Tiêu đề chính (Dòng 1):</label>
              <input
                type="text"
                value={heroConfig.mainHeadingLine1}
                onChange={e => updateHeroField('mainHeadingLine1', e.target.value)}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-mono text-slate-300">Tiêu đề phụ nổi bật (Dòng 2 Gradient):</label>
              <input
                type="text"
                value={heroConfig.mainHeadingLine2}
                onChange={e => updateHeroField('mainHeadingLine2', e.target.value)}
          ...
          {/* Subheading */}
          <div className="space-y-1.5">
            <label className="text-xs font-mono text-slate-300">Đoạn văn mô tả giới thiệu giải pháp:</label>
            <textarea
              rows={3}
        ...
        <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono font-bold text-cyan-400">HỘP THÔNG SỐ 1 (TỐC ĐỘ GIAO)</span>
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <input
                    type="text"
                    value={heroConfig.trustPod1.title}
                    onChange={e => updateTrustPod1('title', e.target.value)}
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={heroConfig.trustPod1.value}
                    onChange={e => updateTrustPod1('value', e.target.value)}
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={heroConfig.trustPod1.sub}
                    onChange={e => updateTrustPod1('sub', e.target.value)}
            ...
            <div className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono font-bold text-emerald-400">HỘP THÔNG SỐ 2 (BẢO LÃNH ESCROW)</span>
                <label className="flex items-center gap-1 text-[11px] font-mono">
                  <input
                    type="checkbox"
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <input
                    type="text"
                    value={heroConfig.trustPod2.title}
                    onChange={e => updateTrustPod2('title', e.target.value)}
                    className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2 text-xs font-mono text-white"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    value={heroConfig.trustPod2.value}
                    onChange={e => updateTrustPod2('value', e.target.value)}
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={heroConfig.trustPod2.sub}
                    onChange={e => updateTrustPod2('sub', e.target.value)}
