...
export const TelcoCardModal: React.FC<TelcoCardModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!serial.trim() || !pin.trim()) {
      alert('Vui lòng nhập đầy đủ Số Serial và Mã Thẻ (PIN)!');
      return;
    }
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base sm:text-lg font-bold font-mono text-white tracking-wide">
                  NẠP THẺ CÀO TỰ ĐỘNG 24/7 (AUTO CARD API)
                </h2>
              ...
              <p className="text-xs text-slate-400 font-mono mt-0.5">
                Hệ thống gạch thẻ tự động qua TheSieuRe / GachThe247 • Tỷ giá tốt nhất thị trường
              </p>
            </div>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                <label className="text-xs font-mono font-bold text-slate-300 flex items-center justify-between">
                  <span>1. CHỌN NHÀ MẠNG / LOẠI THẺ</span>
                  <span className="text-[11px] text-cyan-400">Chiết khấu: {feePercent}%</span>
                </label>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-3 gap-2">
                  {AMOUNTS.map((amt) => (
                    <button
                      ...
                      className={`p-2 rounded-lg border font-mono text-xs text-center transition-all ${
                        selectedAmount === amt
                ...
                <div className="p-2.5 rounded-lg bg-amber-950/30 border border-amber-500/30 text-[11px] font-mono text-amber-300 flex items-center gap-2">
                  ...
                  <span>Chú ý: Chọn sai mệnh giá sẽ bị trừ 50% thực nhận hoặc phạt theo quy định API viễn thông.</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label className="text-xs font-mono text-slate-300">3. SỐ SERIAL</label>
                  <input
                    type="text"
                    value={serial}
                    onChange={(e) => setSerial(e.target.value)}
                    placeholder="Nhập số Serial in trên thẻ..."
                    required
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-mono text-slate-300">4. MÃ THẺ (MÃ PIN CÀO)</label>
                  <input
                    type="text"
                    value={pin}
                    onChange={(e) => setPin(e.target.value)}
                    placeholder="Nhập mã thẻ dưới lớp tráng bạc..."
                    required
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 font-mono text-xs text-white focus:outline-none focus:border-cyan-400"
                  />
                </div>
              ...
              <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 space-y-2 text-xs font-mono">
                ...
                <div className="flex justify-between text-slate-400">
                  <span>Chiết khấu gạch thẻ:</span>
                  <span className="text-red-400 font-bold">-{feePercent}%</span>
                </div>
                <div className="pt-2 border-t border-slate-800 flex justify-between items-center">
                  <span className="text-slate-200 font-bold">Thực nhận vào ví Cyber:</span>
              ...
              >
                {isSubmitting ? (
                  <>
                    ...
                    <span>ĐANG GỬI LÊN CỔNG GẠCH THẺ TỰ ĐỘNG (3S)...</span>
                  </>
                ) : (
                  <>
                    <Zap className="w-4 h-4" />
                    <span>GỬI THẺ CÀO & CỘNG TIỀN VÍ NGAY</span>
                  </>
                )}
              </button>
            </form>
            ...
            <div className="space-y-3">
              {cardHistory.length === 0 ? (
                <div className="p-8 text-center rounded-xl bg-slate-900/30 border border-dashed border-slate-800 text-xs font-mono text-slate-500">
                  Chưa có lịch sử nạp thẻ cào nào. Hãy gửi thẻ đầu tiên của bạn!
                </div>
              ) : (
                  ...
                  <div key={card.id} className="p-3.5 rounded-xl bg-slate-900/50 border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs font-mono">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        ...
                        <div className="text-[10px] text-slate-500">Phí {card.feePercent}%</div>
                      </div>
                      ...
                      <span className={`px-2 py-1 rounded text-[10px] font-bold border uppercase ${
                        ...
                        {card.status === 'success' ? 'Thành công' : card.status === 'processing' ? 'Đang duyệt' : 'Thẻ lỗi'}
                      </span>
                    </div>
                  </div>
                ))
