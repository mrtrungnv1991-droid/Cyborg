...
function AppContent() {
  ...
  // Instant Single Purchase Handler
  const handleInstantBuy = (product: Product) => {
    if (currentUser.walletBalance < product.retailPrice) {
      openModal('wallet');
      return;
    }

    updateUserBalance(-product.retailPrice);
    ...
    const newOrder: UserOrder = {
      ...
      platform: product.platform,
      type: 'instant_single',
      pricePaid: product.retailPrice,
      status: 'fulfilled',
      createdAt: new Date().toLocaleString('vi-VN'),
      deliveredKey: generateRandomKey(product.platform),
      pinCode: '7721',
      giftUpCard: product.deliveryType === 'giftup_card' ? {
        cardNumber: `4928 ${Math.floor(1000 + Math.random() * 9000)} ${Math.floor(1000 + Math.random() * 9000)} ${Math.floor(1000 + Math.random() * 9000)}`,
        pinCode: '7721',
      ...
      txId: `TX-INSTANT-${Date.now().toString().slice(-6)}`
    };

    addTransaction({
      type: 'buy_instant',
      description: `Mua lẻ tức thì: ${product.title}`,
      amount: -product.retailPrice,
      balanceAfter: currentUser.walletBalance - product.retailPrice,
      status: 'completed',
      txCode: newOrder.txId
    });
    ...
    setCelebrationToast({
      ...
      message: `Đơn mua ${product.title} đã hoàn tất. Key đã sẵn sàng trong Kho Key của bạn!`,
      orderId: newOrder.id
    });
  };
  ...
  return (
    <div className="min-h-screen bg-[#050811] text-slate-100 flex flex-col font-sans selection:bg-cyan-500 selection:text-black">
      ...
      {/* 2. Hero Telemetry & Status Engine */}
      <HeroTelemetry 
        user={currentUser}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-10 w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-mono font-bold tracking-widest text-cyan-400 uppercase">
                DIGITAL ASSETS // CHỢ SỈ TÀI NGUYÊN SỐ
              </span>
            </div>
            <h2 className="text-2xl sm:text-3xl font-black text-white uppercase tracking-tight">
              Sản Phẩm Số & Tài Khoản Bản Quyền
            </h2>
            <p className="text-sm text-slate-400 mt-1">
              Gom đơn giá sỉ từ -30% đến -70% với bảo lãnh ký quỹ Escrow tự động
            </p>
          </div>
          ...
          >
            ...
            <span>MỞ NHÓM GOM ĐƠN MỚI</span>
          </button>
        </div>

        {/* Categories & Search Filter Bar */}
        <div className="bg-[#0b1329]/80 border border-slate-800/80 rounded-2xl p-4 mb-8 space-y-4 backdrop-blur-md">
          <div className="flex flex-col lg:flex-row gap-3">
            ...
            <div className="flex-1 relative">
              <input
                ...
                placeholder="Tìm kiếm tài khoản ChatGPT, Canva, Netflix, Spotify, Steam, Key Windows..."
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 lg:pb-0 scrollbar-none">
              {[
                ...
                { id: 'vpn_security', label: 'VPN & An Ninh' },
            ...
            <select
              ...
              className="px-3 py-2.5 rounded-xl bg-slate-900/90 border border-slate-700/60 text-xs font-semibold text-slate-200 focus:outline-none focus:border-cyan-500/80 cursor-pointer"
            >
        ...
        {filteredProducts.length === 0 ? (
          <div className="bg-[#0b1329]/40 border border-slate-800/60 rounded-2xl p-12 text-center">
            ...
            <h3 className="text-lg font-bold text-white mb-1">Không tìm thấy sản phẩm phù hợp</h3>
            <p className="text-sm text-slate-400 mb-4">
              Thử tìm kiếm với từ khóa khác hoặc mở nhóm gom đơn mới để mời bạn bè cùng mua!
            </p>
            <button
